import machine
import neopixel
import time
import math

# --- 硬件引脚配置 ---
RADAR_PIN = 2        # LD2410C 的 OUT 引脚
MOSFET_PIN = 4       # AO3401 PMOS 的栅极 (Gate)
LED_PIN = 5          # WS2812B 的数据引脚 (DIN)
LED_COUNT = 21      # 你的灯珠数量，根据实际修改

# --- 初始化引脚 ---
# PMOS 是低电平导通，初始化为高电平以保持灯带断电
power_gate = machine.Pin(MOSFET_PIN, machine.Pin.OUT, value=1)
radar = machine.Pin(RADAR_PIN, machine.Pin.IN)
np = neopixel.NeoPixel(machine.Pin(LED_PIN), LED_COUNT)

def set_all_color(r, g, b):
    """设置所有灯珠颜色"""
    for i in range(LED_COUNT):
        np[i] = (r, g, b)
    np.write()

def progress_bar_effect():
    """进度条效果：每个灯珠间隔10ms点亮"""
    for i in range(LED_COUNT):
        np[i] = (255, int(255 * 0.6), int(255 * 0.2))  # 暖白色
        np.write()
        time.sleep_ms(10)

def breathing_once():
    """完成一次呼吸效果（亮→暗→亮→暗）"""
    steps = 100
    # 亮→暗
    for i in range(steps):
        brightness = int((1 - i / steps) * 255)
        set_all_color(brightness, int(brightness * 0.6), int(brightness * 0.2))
        time.sleep_ms(10)
    # 暗→亮
    for i in range(steps):
        brightness = int(i / steps * 255)
        set_all_color(brightness, int(brightness * 0.6), int(brightness * 0.2))
        time.sleep_ms(10)
    # 亮→暗
    for i in range(steps):
        brightness = int((1 - i / steps) * 255)
        set_all_color(brightness, int(brightness * 0.6), int(brightness * 0.2))
        time.sleep_ms(10)

def off_lights():
    """彻底关闭灯带电源"""
    set_all_color(0, 0, 0) # 先软关闭信号
    power_gate.value(1)    # 切断 PMOS 电源

def on_lights():
    """开启灯带电源"""
    power_gate.value(0)    # PMOS 导通
    time.sleep_ms(50)      # 等待电压稳定

# --- 主循环 ---
print("系统启动，等待上升沿触发...")

last_state = radar.value()

try:
    while True:
        current_state = radar.value()

        # 检测上升沿（低→高）
        if last_state == 0 and current_state == 1:
            print("检测到上升沿！执行灯效...")
            on_lights()
            progress_bar_effect()  # 进度条效果
            breathing_once()           # 一次呼吸效果
            off_lights()               # 关闭灯光
            print("灯效完成，等待下一次触发...")

        last_state = current_state
        time.sleep_ms(20)# 短暂延时，避免CPU占用过高

except KeyboardInterrupt:
    print("程序停止")
    off_lights()